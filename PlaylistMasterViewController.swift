...
class PlaylistMasterViewController: UIViewController {
    ...
    let playlistDetailSegueIdentifier = "showPlaylistDetailSegue"
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if let index = find(playlistArray, playlistImageView) {
            if segue.identifier == playlistDetailSegueIdentifier {
    ...
    @IBAction func showPlaylistDetail(sender: AnyObject) {
        ...
        performSegueWithIdentifier(playlistDetailSegueIdentifier, sender: sender)
    }
}
